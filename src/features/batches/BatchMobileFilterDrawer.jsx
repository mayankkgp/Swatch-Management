...
import {
  ...
  DateAccordion,
  AccordionSection
} from './BatchMobileFilterDrawerAccordions';

export default function BatchMobileFilterDrawer({
  isOpen,
  onClose,
  selectedVendors,
  setSelectedVendors,
  ...
  const [tempMinSwatches, setTempMinSwatches] = useState(filterMinSwatches);
  const [tempMonth, setTempMonth] = useState(filterMonth);
  const [openSection, setOpenSection] = useState('vendor'); // 'vendor' | 'date' | 'minSwatches' | 'month'
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTempMinSwatches(filterMinSwatches);
      setTempMonth(filterMonth);
    }
  }, [isOpen, selectedVendors, fromDate, toDate, filterMinSwatches, filterMonth]);

  const handleClearAll = () => {
    setTempVendors([]);
    setTempFromDate('');
    setTempToDate('');
    setTempMinSwatches('all');
    setTempMonth('all');
  };
  ...
  if (tempMinSwatches !== 'all') {
    tempPills.push({
      id: 'min-swatches',
      label: `Min Swatches: ${tempMinSwatches}+`,
      onDismiss: () => setTempMinSwatches('all')
    });
  }
  if (tempMonth !== 'all') {
    tempPills.push({
      id: 'month',
      label: `Month: ${tempMonth === '06' ? 'June 2026' : 'July 2026'}`,
      onDismiss: () => setTempMonth('all')
    });
  }
